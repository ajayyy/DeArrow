...
import Config, { ConfigurationID, ThumbnailFallbackAutogeneratedOption, ThumbnailFallbackOption, TitleFormatting } from "./config";
...
export interface ChannelOverrideFetchOptions {
    onlyVideo?: boolean;
    onlyChannelID?: boolean;
}

export interface VideoOverrideData {
    videoID: VideoID;
...
export function getChannelOverrideID(data: VideoOverrideData | null, options?: ChannelOverrideFetchOptions): ConfigurationID | null {
    if (!data?.videoID) return null;

    const overrideTypes = getUsedOverrideTypes();
    const channelIdentifiers: string[] = [];

    if (!options?.onlyChannelID) {
        channelIdentifiers.push(data.videoID);
    }

    if (!options?.onlyVideo) {
        if (overrideTypes.name) {
            const id = data.channelName;
            if (id != null) channelIdentifiers.push(id);
        }
        if (overrideTypes.channelID) {

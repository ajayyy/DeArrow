        ...
        try {
            const thumbnail = await createThumbnailImageElement(existingImageElement, videoID, width, height, brandingLocation, timestamp ?? null, false, async () => {
                ...
                if (await getThumbnailFallbackOption(videoID) === ThumbnailFallbackOption.RandomTime) {
                    await resetToShowAutogenerated(videoID, thumbnail, displayThumbnail);
                } else {
                    resetToShowOriginalThumbnail(image, brandingLocation);
                }
            });
...
async function resetToShowAutogenerated(videoID: VideoID, thumbnail: HTMLImageElement | null,
